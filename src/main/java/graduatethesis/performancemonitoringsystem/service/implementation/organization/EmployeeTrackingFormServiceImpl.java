...
public class EmployeeTrackingFormServiceImpl implements EmployeeTrackingFormService {
    ...
    public List<ReportTotalTasksHelper> findAllOrderedByOrganizationalDepartment() {
        List<EmployeeTrackingForm> employeeTrackingForms = this.employeeTrackingFormRepository.findAllOrderedByOrganizationalDepartment();
        List<ReportTotalTasksHelper> reportTotalTasksHelpers=new ArrayList<>();
        int count = employeeTrackingForms.get(0).getValue();
        for(int i=0;i<employeeTrackingForms.size();i++) {
            ReportTotalTasksHelper reportTotalTasksHelper = new ReportTotalTasksHelper();
            EmployeeTrackingForm prevEmployeeTrackingForm = employeeTrackingForms.get(i);

            if(employeeTrackingForms.size()==1){
                reportTotalTasksHelper.setWorkingItem(prevEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getWorkingItem().getName());
                reportTotalTasksHelper.setStatus(prevEmployeeTrackingForm.getStatus().getName());
                reportTotalTasksHelper.setOrganizationalDepartment(prevEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getOrganizationalDepartment().getName());
                reportTotalTasksHelper.setCountForTasks(count);
                reportTotalTasksHelpers.add(reportTotalTasksHelper);
                return reportTotalTasksHelpers;
            }
            EmployeeTrackingForm nextEmployeeTrackingForm = employeeTrackingForms.get(i + 1);

            if(prevEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getWorkingItem().getName().equals(nextEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getWorkingItem().getName()) && prevEmployeeTrackingForm.getStatus().getName().equals(nextEmployeeTrackingForm.getStatus().getName())
                    && prevEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getOrganizationalDepartment().getName().equals(nextEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getOrganizationalDepartment().getName())){
                count += nextEmployeeTrackingForm.getValue();
            }

            else  {
                if (!reportTotalTasksHelpers.contains(reportTotalTasksHelper)) {
                    reportTotalTasksHelper.setWorkingItem(prevEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getWorkingItem().getName());
                    reportTotalTasksHelper.setStatus(prevEmployeeTrackingForm.getStatus().getName());
                    reportTotalTasksHelper.setOrganizationalDepartment(prevEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getOrganizationalDepartment().getName());
                    reportTotalTasksHelper.setCountForTasks(count);
            ...
            if(i+1==employeeTrackingForms.size()-1) {
                ReportTotalTasksHelper reportTotalTasksHelper1=new ReportTotalTasksHelper();
                reportTotalTasksHelper1.setWorkingItem(employeeTrackingForms.get(employeeTrackingForms.size() - 1).getOrganizationalDepartmentWorkingItem().getWorkingItem().getName());
                reportTotalTasksHelper1.setStatus(employeeTrackingForms.get(employeeTrackingForms.size() - 1).getStatus().getName());
                reportTotalTasksHelper1.setOrganizationalDepartment(employeeTrackingForms.get(employeeTrackingForms.size() - 1).getOrganizationalDepartmentWorkingItem().getOrganizationalDepartment().getName());
                reportTotalTasksHelper1.setCountForTasks(count);
                if (!reportTotalTasksHelpers.contains(reportTotalTasksHelper1)) {
                    reportTotalTasksHelpers.add(reportTotalTasksHelper1);
                }
                break;
            }
    ...
    public List<ReportTotalTasksByEmployeeHelper> findAllOrderedByEmployee() {
        List<EmployeeTrackingForm> employeeTrackingForms = this.employeeTrackingFormRepository.findAllOrderedByEmployee();
        List<ReportTotalTasksByEmployeeHelper> reportTotalTasksByEmployeeHelpers = new ArrayList<>();
        int count = employeeTrackingForms.get(0).getValue();
        for (int i = 0; i < employeeTrackingForms.size(); i++) {
            ReportTotalTasksByEmployeeHelper reportTotalTasksByEmployeeHelper = new ReportTotalTasksByEmployeeHelper();
            EmployeeTrackingForm prevEmployeeTrackingForm = employeeTrackingForms.get(i);

            if(employeeTrackingForms.size()==1){
                ReportTotalTasksByEmployeeHelper reportTotalTasksByEmployeeHelper1 = new ReportTotalTasksByEmployeeHelper();
                reportTotalTasksByEmployeeHelper1.setWorkingItem(employeeTrackingForms.get(employeeTrackingForms.size() - 1).getOrganizationalDepartmentWorkingItem().getWorkingItem().getName());
                reportTotalTasksByEmployeeHelper1.setStatus(employeeTrackingForms.get(employeeTrackingForms.size() - 1).getStatus().getName());
                reportTotalTasksByEmployeeHelper1.setFirsName(employeeTrackingForms.get(employeeTrackingForms.size() - 1).getUser().getPerson().getFirstName());
                reportTotalTasksByEmployeeHelper1.setLastName(employeeTrackingForms.get(employeeTrackingForms.size() - 1).getUser().getPerson().getLastName());
                reportTotalTasksByEmployeeHelper1.setCountForTasks(count);
                reportTotalTasksByEmployeeHelpers.add(reportTotalTasksByEmployeeHelper1);
                return reportTotalTasksByEmployeeHelpers;
            }
            EmployeeTrackingForm nextEmployeeTrackingForm = employeeTrackingForms.get(i + 1);
            ...
            if (prevEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getWorkingItem().getName().equals(nextEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getWorkingItem().getName()) && prevEmployeeTrackingForm.getStatus().getName().equals(nextEmployeeTrackingForm.getStatus().getName())) {
                ...
                if (!reportTotalTasksByEmployeeHelpers.contains(reportTotalTasksByEmployeeHelper)) {
                    reportTotalTasksByEmployeeHelper.setWorkingItem(prevEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getWorkingItem().getName());
                    reportTotalTasksByEmployeeHelper.setStatus(prevEmployeeTrackingForm.getStatus().getName());
                    reportTotalTasksByEmployeeHelper.setFirsName(prevEmployeeTrackingForm.getUser().getPerson().getFirstName());
                    reportTotalTasksByEmployeeHelper.setLastName(prevEmployeeTrackingForm.getUser().getPerson().getLastName());
                    reportTotalTasksByEmployeeHelper.setCountForTasks(count);
                    count = nextEmployeeTrackingForm.getValue();
                    reportTotalTasksByEmployeeHelpers.add(reportTotalTasksByEmployeeHelper);
                }
            ...
            if (i + 1 == employeeTrackingForms.size() - 1) {
                ...
                reportTotalTasksByEmployeeHelper1.setLastName(employeeTrackingForms.get(employeeTrackingForms.size() - 1).getUser().getPerson().getLastName());
                reportTotalTasksByEmployeeHelper1.setCountForTasks(count);
                if (!reportTotalTasksByEmployeeHelpers.contains(reportTotalTasksByEmployeeHelper1)) {
                    reportTotalTasksByEmployeeHelpers.add(reportTotalTasksByEmployeeHelper1);
                }
                break;
            }
    ...
    @Override
    public List<ReportTasksByTimeHelper> findAllByTime(DateHelper dateHelper) {
        List<ReportTasksByTimeHelper> reportTasksByTimeHelperList = new ArrayList<>();
        List<EmployeeTrackingForm> lista = this.employeeTrackingFormRepository.findAllByTime(dateHelper);
        List<EmployeeTrackingForm> employeeTrackingFormList = new ArrayList<>();

        for (int i = 0; i < lista.size(); i++) {
            if (lista.get(i).getStatus().getName().equals("done")) {
                employeeTrackingFormList.add(lista.get(i));
            }
        }

        if(employeeTrackingFormList.size()==0){
            throw new IsEmptyListException();
        }else{
            int count = employeeTrackingFormList.get(0).getValue();
            for (int i = 0; i < lista.size(); i++) {
                ReportTasksByTimeHelper reportTasksByTimeHelper = new ReportTasksByTimeHelper();
                if (i == employeeTrackingFormList.size() - 1) {
                    if (employeeTrackingFormList.get(i).getUser().getId().equals(employeeTrackingFormList.get(i).getUser().getId())) {
                        reportTasksByTimeHelper.setWorkingItem(employeeTrackingFormList.get(i).getOrganizationalDepartmentWorkingItem().getWorkingItem().getName());
                        reportTasksByTimeHelper.setStatus(employeeTrackingFormList.get(i).getStatus().getName());
                        reportTasksByTimeHelper.setFirstName(employeeTrackingFormList.get(i).getUser().getPerson().getFirstName());
                        reportTasksByTimeHelper.setLastName(employeeTrackingFormList.get(i).getUser().getPerson().getLastName());
                        reportTasksByTimeHelper.setStartDate(employeeTrackingFormList.get(i).getTaskStartDate());
                        reportTasksByTimeHelper.setEndDate(employeeTrackingFormList.get(i).getTaskEndDate());
                        reportTasksByTimeHelper.setCount(count);
                        reportTasksByTimeHelperList.add(reportTasksByTimeHelper);
                        break;
                    } else {
                        break;
                    }
                }
                EmployeeTrackingForm nextEmployeeTrackingForm = employeeTrackingFormList.get(i + 1);


                if (employeeTrackingFormList.get(i).getUser().getId().equals(nextEmployeeTrackingForm.getUser().getId())) {
                    if (employeeTrackingFormList.get(i).getOrganizationalDepartmentWorkingItem().getWorkingItem().getName().equals(nextEmployeeTrackingForm.getOrganizationalDepartmentWorkingItem().getWorkingItem().getName())) {
                        count += nextEmployeeTrackingForm.getValue();
                ...
                if (i + 1 == employeeTrackingFormList.size() - 1) {
                    if (reportTasksByTimeHelperList.isEmpty()) {
                        reportTasksByTimeHelper.setWorkingItem(employeeTrackingFormList.get(i).getOrganizationalDepartmentWorkingItem().getWorkingItem().getName());
                        reportTasksByTimeHelper.setStatus(employeeTrackingFormList.get(i).getStatus().getName());
                        reportTasksByTimeHelper.setFirstName(employeeTrackingFormList.get(i).getUser().getPerson().getFirstName());
                        reportTasksByTimeHelper.setLastName(employeeTrackingFormList.get(i).getUser().getPerson().getLastName());
            ...
            return reportTasksByTimeHelperList;
        }
